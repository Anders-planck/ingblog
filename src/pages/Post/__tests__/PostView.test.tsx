...
describe('Post View Page', () => {
  ...
  describe('check if postView is render well and user is logged in ', () => {
    beforeEach(() => {
      (useGetPostQuery as jest.Mock).mockImplementation(({ postId }) => ({
        data: postsFake.find((post) => post.id === postId) ?? null,
        isLoading: false,
      }));
    ...
    it('when in the params we have good id post', async () => {
      let getByTestIdMaster: any;
      let containerMaster: any;

      await waitFor(async () => {
        const { container, getByTestId } = renderWithProviders(<PostView />);
        getByTestIdMaster = getByTestId;
        containerMaster = container;
      });
      // verify if useParams is called and return the good id
      expect(useParams).toHaveBeenCalled();
      expect(useParams).toHaveReturnedWith({ id: '1' });

      // verify if useGetPostQuery is called and return the good post
      expect(useGetPostQuery).toHaveBeenCalled();
      expect(useGetPostQuery).toHaveReturnedWith({
        data: postsFake[0],
        isLoading: false,
      });
      ...
      expect(getByTestIdMaster('post-view')).toBeInTheDocument();

      expect(containerMaster).toHaveTextContent(postsFake[0].title);
      expect(containerMaster).toHaveTextContent(postsFake[0].overview);

      // verify if the author is render well
      expect(containerMaster).toHaveTextContent(postsFake[0].author.name);

      if (postsFake[0].likes) {
        expect(containerMaster).toHaveTextContent(`${postsFake[0].likes.length} likes`);
      }

      if (postsFake[0].comments) {
        const comments = getByTestIdMaster('comments-list');
        const commentForm = getByTestIdMaster('comment-form');

        expect(comments).toBeInTheDocument();
        expect(commentForm).toBeInTheDocument();
        expect(containerMaster).toHaveTextContent('Comments');
        expect(containerMaster).toHaveTextContent(postsFake[0].comments[0].content);
      }
    });
  });

  describe('check if postView is render well and user is logged in and id is wrong', () => {
    beforeEach(() => {
      (useGetPostQuery as jest.Mock).mockImplementation(({ postId }) => ({
        data: postsFake.find((post) => post.id === postId) ?? null,
        isLoading: false,
      }));
      (useParams as jest.Mock).mockReturnValue({ id: '45' });
      (useAppSelector as jest.Mock).mockImplementation((selector) => {
        if (selector === selectUser) {
          return userFake;
        }
    ...
    it('when in the params we have good id post', async () => {
      ...
      // verify if useParams is called and return the good id
      expect(useParams).toHaveBeenCalled();
      expect(useParams).toHaveReturnedWith({ id: '45' });
      ...
      expect(getByTestIdMaster('post-view-not-fount')).toBeInTheDocument();
      expect(containerMaster).toHaveTextContent('Post not found');
    });
  });

  describe('check if postView is render well and user is not logged in and id is good', () => {
    beforeEach(() => {
      (useGetPostQuery as jest.Mock).mockImplementation(({ postId }) => ({
        data: postsFake.find((post) => post.id === postId) ?? null,
        isLoading: false,
      ...
      (useParams as jest.Mock).mockReturnValue({ id: '1' });
      (useAppSelector as jest.Mock).mockImplementation(() => null);
    });
    ...
    it('when in the params we have good id post', async () => {
      ...
      expect(useAppSelector).toHaveBeenCalled();
      expect(useAppSelector).toHaveReturnedWith(null);
